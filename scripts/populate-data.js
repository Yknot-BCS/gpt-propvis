...
function formatPropertyAsCode(property: Property, index: number): string {
  const isLast = index === 0; // This will be handled in the main function
  return `  {
    id: "${property.id}",
...
async function main() {
  try {
    ...
    const propertiesCode = properties.map((property, index) => 
      formatPropertyAsCode(property, index)
    ).join(',\n');
